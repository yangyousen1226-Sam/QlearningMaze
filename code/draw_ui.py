import matplotlib
matplotlib.use("Qt5Agg")  # Declare to use Qt5 backend for matplotlib
from matplotlib.backends.backend_qt5agg import FigureCanvasQTAgg as FigureCanvas
from matplotlib.figure import Figure
import numpy as np
from maze import RIGHT, UP, LEFT, DOWN

# Class for drawing maze UI components using matplotlib with Qt5 backend
class Draw_ui(FigureCanvas):
    # Initialize canvas with specified size and dpi, create two subplots for q-table and maze path
    def __init__(self, width=5, height=4, dpi=100):
        self.fig = Figure(figsize=(width, height), dpi=dpi)
        super(Draw_ui,self).__init__(self.fig)
        self.axes_table = self.fig.add_subplot(121)  # Subplot for Q-table visualization
        self.axes_table.set_xticklabels([])
        self.axes_table.set_yticklabels([])
        self.axes_root = self.fig.add_subplot(122)   # Subplot for maze path/root visualization
        self.axes_root.set_xticklabels([])
        self.axes_root.set_yticklabels([])

    # Draw Q-table visualization with maze elements, actions and current time
    def draw_qtable(self, qtable_model, time_, fire_flag):
        ...
        aim_pos = qtable_model.my_maze.aim  # Get exit position of maze

        # Scale markers based on maze size
        size_factor = 7.0 / qtable_model.my_maze.maze.shape[0]
        # Draw exit position (green square with "Exit" text)
        self.axes_table.plot(aim_pos[1], aim_pos[0], color='green', marker='s',
                markersize=int(30 * size_factor))
        ...
        # Draw arrows for optimal actions in each free cell (color by certainty)
        for cell in qtable_model.my_maze.free_cells:
            q = qtable_model.q_value((*cell, time_)) if qtable_model is not None else [0, 0, 0, 0]
            a = np.nonzero(q == np.max(q))[0]  # Get optimal actions for current cell/time

            for action in a:
                dx = 0
                dy = 0
                # Set direction offset based on action
                if action == LEFT:
                    dx = -0.2
                if action == RIGHT:
                    dx = +0.2
        ...
        # Refresh canvas to show updates
        self.axes_table.get_figure().canvas.draw()

    # Draw maze path visualization with start/exit, visited cells and current time
    def draw_root(self, my_maze, start_pos, time_, period, fire_flag):
        ...
        aim_pos = my_maze.aim  # Get exit position

        # Scale markers based on maze size
        size_factor = 7.0 / my_maze.maze.shape[0]
        # Draw exit position (green square with "Exit" text)
        self.axes_root.plot(aim_pos[1], aim_pos[0], color='green', marker='s',markersize=int(30 * size_factor))
        self.axes_root.text(aim_pos[1], aim_pos[0], "Exit", ha="center", va="center", color="white", fontSize=int(10*size_factor))

        # Draw start position (brown square with "Start" text)
        ...
        # Display current time (mod period if period is set)
        self.axes_root.text(0, my_maze.maze.shape[0], "time:" + str(time_ % period if period != 0 else time_), ha="center",
                va="center", color="black")

        # Draw visited cells: yellow circles (path) and blue star (current mouse position)
        for i, cell in enumerate(my_maze.visited):
            if i + 1 >= time_ and time_ > 0:
                ...
                break
            self.axes_root.plot(cell[1], cell[0], color='yellow', marker='o',markersize=int(25 * size_factor))

        # Draw maze elements: traps, fire (if enabled), blocks
    ...
    # Draw blocked cells (walls) in maze using binary colormap
    ...
    # Draw trap cells (red square with "Trap" text)
    ...
    # Draw fire cells (red triangle with "Fire" text)
